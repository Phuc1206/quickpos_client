...
const sendGetCustomerDetailRequest = async (customerId: string) => {
	try {
		const response = await api.get(
			`${apiStrings.customer.detail}/${customerId}`,
		);
...
const sendGetCustomerListRequest = async (payload: IPagination) => {
	try {
		const response = await api.get(apiStrings.customer.list, {
			params: payload,
		});
		// console.log("Get customer list request api:", response);
		return response;
	} catch (error) {
		console.error("Get customer list request failed:", error);
		throw error;
	}
};
